...
function plot_gf_dual(dfl, dfr)
    ...
    xlabels2 = collect(1.2:0.2:2)
    p2 = plot(dfr, x=:gf, y=:N, color=:Color, Geom.step,
        Theme(plot_padding=[2mm, 2mm, 2mm, 2mm]), 
        Guide.xlabel("Apparent Growth Factor (-)"),
        Guide.ylabel("Concentration (cm⁻³)", orientation=:vertical),
        Guide.xticks(ticks=collect(1.2:0.1:2.0)),
...
function f01(Dd, k, gf0)
	...
	DMA₁1(𝕟, zˢ, gf) = @_ map((gf ⋅ (T₁(zˢ, _) * 𝕟)), 1:6)
	DMA₂1(𝕟, k) =  (O(k) * 𝕟)
	ℕ1 = DMA₁1(𝕟ᶜⁿ, zˢ, gf[argmin(abs.(gf .- gf0))])
	𝕄1= map(k -> (@> itp(ℕ1[k]) DMA₂1(k)), 1:6)
	dfl31 = DataFrame(gf=𝕄1[1].Dp ./ (Dd * 1e9), N=𝕄1[1].N, Color="k = 1")
    dfl41 = DataFrame(gf=𝕄1[2].Dp ./ (Dd * 1e9), N=𝕄1[2].N, Color="k = 2")
    dfl51 = DataFrame(gf=𝕄1[3].Dp ./ (Dd * 1e9), N=𝕄1[3].N, Color="k = 3")
	dfl61 = DataFrame(gf=sum(𝕄1).Dp ./ (Dd * 1e9), N=sum(𝕄1).N, Color="k = 8")
	dfr = [dfl2; dfl31;dfl41;dfl51;dfl61]
	return plot_gf_dual(dfr, dfl) 
end

set_default_plot_size(18cm, 7cm)
@>> f01(100e-9, 60, 1.6) Gadfly.draw(PNG("figures/response.png", dpi = 300))
